...
static inline int i2c_smbus_access (int fd, char rw, uint8_t command, int size, union i2c_smbus_data *data)
{
  ...
  args.size       = size ;
  args.data       = data ;
 // printf("i2c_smbus_access is %x\n",data);
//  printf("ioctl i is :%x\n",ioctl (fd,I2C_SMBUS,&args));
  rc =  ioctl (fd, I2C_SMBUS, &args) ;
  printf("rc is : %d\n",rc);
 if(rc < 0 )
  { 
    printf("wirte,I2C_RDWR failed:%d.\n",errno);
    close(fd);
    return -1;
  }
  printf("write value is : 0x%02x\n",args.data->byte);
  return rc;
}
...
int wiringPiI2CWriteReg8 (int fd, int reg, int value)
{
  ...
  data.byte = value ;
  printf("wiringPiI2CWriteReg8 value : %x\n ",data.byte);
...
int wiringPiI2CSetup (const int devId)
{
  ...
  printf("devId is : %d:%x",devId,devId);
  rev = piGpioLayout () ;
